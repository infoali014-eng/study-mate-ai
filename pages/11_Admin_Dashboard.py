...
with metric_cols[0]:
    render_stat_card("Users", counts["users"], "Registered accounts", "users", "#14b8b4", "#d8fff6")
with metric_cols[1]:
    render_stat_card("Subjects", counts["subjects"], "Across all users", "book", "#2f7df6", "#e3efff")
with metric_cols[2]:
    render_stat_card("Documents", counts["documents"], "Uploaded materials", "file-text", "#ff637d", "#ffe3e9")
with metric_cols[3]:
    render_stat_card("Quizzes", counts["quizzes"], "Total attempts", "help-circle", "#8b5cf6", "#efe7ff")

metric_cols2 = st.columns(4)
with metric_cols2[0]:
    render_stat_card("Flashcards", counts["flashcards"], "Saved cards", "layers", "#ffb703", "#fff3c4")
with metric_cols2[1]:
    render_stat_card("Revision Plans", counts["revision_plans"], "Saved plans", "calendar", "#58c84f", "#e8ffd9")
with metric_cols2[2]:
    render_stat_card("Storage", f"{_storage_mb():.1f} MB", "Local data folder", "upload-cloud", "#2f7df6", "#e3efff")
with metric_cols2[3]:
    render_stat_card("AI Provider", ai_engine.get_selected_provider(), "Current session", "zap", "#14b8b4", "#d8fff6")

left, right = st.columns(2)
with left:
    section_title("Recent Users", "users")
...
with right:
    section_title("Recent Uploads", "file-text")
...
section_title("App Health", "settings")
with st.container(border=True):
